...
\author{Nils Matteson\\\small\texttt{nomatteson@wisc.edu}}
\date{STAT 479 Group Project 1 \\ Spring 2026 \\[0.4em]
\small\textit{Submitted as a group project; sole-author writeup. See
...
1982. After 1:1 nearest-neighbor matching on a Mahalanobis-plus-propensity
distance, all nine measured confounders satisfy the conventional
$|\mathrm{SMD}| < 0.10$ balance criterion, with most below $0.05$.
The matched estimate is $\hat{\tau} \approx 3.20$ kg
(95\% CI $[2.16,\,4.24]$), meaning quitters gained roughly three
...
small mismatch between that target and the design we ran. A 1:1
nearest-neighbor design that matches every treated unit to a control most
naturally identifies the ATT, the average effect on the treated:
$$
\tau_{\text{ATT}}
\;=\;
\mathbb{E}\!\left[\,Y(1) - Y(0)\,\bigm|\,Z = 1\right].
$$
ATE and ATT coincide when the treatment effect is constant across the
covariate space, and they tend to be close in practice when overlap is
strong. We report $\hat\tau$ as a paired estimate on the matched sample
and discuss both interpretations in Section~\ref{sec:identification}.
...
relative to the matched estimate because quitters skew older and heavier-
smoking at baseline, and both correlate negatively with weight change.
...
We match every treated unit (quitter) to one control using a distance
...
The match returns 403 pairs, one for every treated respondent. Because
matching is 1:1 and every treated unit finds a partner, the matched-pair
estimand corresponds most cleanly to the ATT, as discussed in
...
On the 403 matched pairs we compute the simple paired
...
\midrule
Matched paired difference & $3.202$ & $0.530$ & $2.163$ & $4.242$ & $403$ \\
\bottomrule
\end{tabular}
...
two perturbations: (a) switching from 1:1 to 1:2 nearest-neighbor matching,
...
\midrule
1:1 NN matching (primary)        & $3.202$ & $0.530$ & $2.163$ & $4.242$ & \\
1:2 NN matching                  & $2.752$ & $0.478$ & $1.816$ & $3.687$ & \\
...
and pulls the estimate down slightly, while trimming the PS support
...
features of the 1:1 design or from the small number of observations near
