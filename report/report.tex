...
\midrule
sex            & $\phantom{-}0.160$ & $\phantom{-}0.022$ \\
age            & $-0.282$           & $-0.125$           \\
race           & $\phantom{-}0.177$ & $\phantom{-}0.023$ \\
education      & $-0.088$           & $-0.045$           \\
smokeintensity & $\phantom{-}0.217$ & $\phantom{-}0.146$ \\
smokeyrs       & $-0.159$           & $-0.053$           \\
exercise       & $-0.104$           & $-0.046$           \\
active         & $-0.089$           & $-0.063$           \\
wt71           & $-0.133$           & $-0.097$           \\
\bottomrule
\end{tabular}
...
the line. After matching, most covariates move closer to zero, but
\texttt{age} and \texttt{smokeintensity} remain slightly above the
conventional threshold, at about $0.125$ and $0.146$ in absolute value.
That is improved balance, not perfect balance, and it is the main reason
we treat the sensitivity analysis as part of the substantive result rather
than a decorative appendix.
...
Most covariates move substantially toward zero after matching, though
\texttt{age} and \texttt{smokeintensity} remain just outside the
threshold.}
\label{fig:love}
\end{figure}

The matched sample is materially better balanced than the raw data, but it
does not fully eliminate measured imbalance on every covariate.
...
So quitting smoking is associated with about $2.75$ kg (close to six
...
recomputing anything inline. The unreleased \texttt{approxmatch} sources are vendored in
